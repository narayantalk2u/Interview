 ...
 * Runtime complexity #
 *
 * Since we are using binary search, the runtime complexity is logarithmic, O(logn)O(logn)O(logn).
 *
 *     Even though we do the binary search twice, asymptotic runtime complexity is still O(logn)O(logn)O(logn).
 *
 * Memory complexity #
 *
 * The memory complexity is constant, O(1)O(1)O(1) since no extra storage is being used.
 *
 *     Note: If the binary search was implemented recursively, there would be implicit O(logn)O(logn)O(logn) memory
 *     usage on the function call stack. We, however, will be focusing on the iterative solution here.
 *
 * Linearly scanning the sorted array for low and high indices are highly inefficient since our array size can be in
 * millions.
 *
