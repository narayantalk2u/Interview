...
public class SearchRotated {
     ...
     * Description #
     *
     * Search for a given number in a sorted array, with unique elements, that has been rotated by some arbitrary
     * number. Return -1 if the number does not exist.
     *
     ...
     * closely, we notice that at least one half of the array is always sorted. We can use this property to our
     * advantage. If the number nnn lies within the sorted half of the array, then our problem is a basic binary search.
     * Otherwise, discard the sorted half and keep examining the unsorted half. Since we are partitioning the array in
     * half at each step, this gives us O(log n)O(log \space n)O(log n) runtime complexity.
     */
